...
import {addToCart,fetchProductRequest} from '../../actions/CartActions'
...
class Shop extends Component{
    ...
    render(){
    var {keyword,currentPage,postPerPage,productData} = this.state
    var {_todoProduct,addToCart} = this.props
        // console.log(_todoProduct); //{cart:[], numberCart : 0,_products:[{},..]}
    //get current post
    const indexOfLastPost = currentPage * postPerPage;
    const indexOfFirstPost = indexOfLastPost - postPerPage;
    const currentPost = productData.slice(indexOfFirstPost,indexOfLastPost);
    ...
    const filterByKeyword = productData.filter(product => {
...
const mapStateToProps = (state)=>{
    return{
        _todoProduct : state.cartReducer
    }
}
